...
void Object::draw(int loc_u_pvm_matrix, int loc_a_vertex, int loc_a_normal,
  ...
  glm::mat4 mat_PV)
{
  ...
  // std::cout<<group.m_vertices.size()<<std::endl;
// std::cout<<"================="<<std::endl;
    ...
    //glVertexAttribPointer(loc_a_texcoord, 2,GL_FLOAT,false,0,group.m_vertices.data());
...
bool Object::load_simple_obj(const std::string& filename)
{
  ...
  PATH = filename.substr(filename.find_last_of('/'), filename.find_last_of('.'));
  ...
  std::stringstream ss;
    ...
    else if (line.substr(0, 2) == "o ")   // group
    {
...
void Object::boundingBox(int loc_a_vertex,int loc_u_pvm_matrix,glm::mat4 mat_PVM)
{
  glm::vec3 max,min;
  max=glm::vec3(0);
  min=glm::vec3(0);
    ...
    for (size_t i=0; i<m_groups.size(); ++i)
    {
      ...
      for(int j=0;j<group.m_vertices.size()/3;j++)
      {
        if(group.m_vertices[j][0]>max[0])
          max[0]=group.m_vertices[j][0];
        if(group.m_vertices[j][1]>max[1])
            max[1]=group.m_vertices[j][1];
        if(group.m_vertices[j][2]>max[2])
          max[2]=group.m_vertices[j][2];
        if(group.m_vertices[j][0]<min[0])
          min[0]=group.m_vertices[j][0];
        if(group.m_vertices[j][1]<min[1])
            min[1]=group.m_vertices[j][1];
        if(group.m_vertices[j][2]<min[2])
            min[2]=group.m_vertices[j][2];
    ...
    GLubyte myList[]={0,1,0,3,1,2,1,7,7,6,7,4,4,0,4,5,5,3,5,6,6,2};
    ...
    glVertexAttribPointer(loc_a_vertex, 4, GL_FLOAT, GL_FALSE, 0, &box);
      glEnableVertexAttribArray(loc_a_vertex);
        glDrawElements(GL_LINES,24,GL_UNSIGNED_BYTE,myList);
      glDisableVertexAttribArray(loc_a_vertex);
}
void Object::o_translate(float x, float y, float z)
{
	T = glm::translate(glm::vec3(x,y,z));
  M *= T;
	cenPos[0] = x;
	cenPos[1] = y;
	cenPos[2] = z;
}
void Object::o_rotate(float angle, float x, float y, float z)
{
	R = glm::rotate(angle,glm::vec3(x,y,z));
  M *= R;
  ang = angle;
  roPos[0] = x;
  roPos[1] = y;
...
void Object::o_scale(float x, float y, float z)
{
	S = glm::scale(glm::vec3(x,y,z));
  M *= S;
  scaPos[0] = x;
  scaPos[1] = y;
  scaPos[2] = z;
}
glm::vec3 Object::CenPos()
{
	return cenPos;
}
glm::vec3 Object::RoPos()
{
	return roPos;
}
glm::vec3 Object::ScaPos()
{
	return scaPos;
}
